...
class ConversionConfiguration: # ConversionTracker?
	def __init__(self):
		self.style2heading = {'Kop1':'h1', 'Heading1':}#{'Kop1':'chapter', 'Heading1':'chapter', 'Kop2':'section', 'Heading2':'section', 'Kop3':'subsection', 'Heading3':'subsection', 'Kop4':'subsubsection', 'Heading4':'subsubsection', 'Kop5':'paragraph', 'Heading5':'paragraph', 'Kop7':'subparagraph', 'Heading7':'subparagraph', 'Kop9':'sssssssSection', 'Heading9':'sssssssSection', 'ZinkHeading1':'chapter', 'ZinkHeading2':'section', 'ZinkHeading3':'subsection', 'ZinkHeading4':'subsubsection'}
		self.style2tag = {}
		self.style2attrib = {}
		...
		self.xmlcode = XMLCode()

	def newChapter(self):
		self.currentChapter += 1
		self.currentTable = 0
		self.xmlcode.newChapter("Chapter{:d}".format(self.currentChapter))
	
	def startTable(self, suppress=False):
		self.inTable = True
		if not suppress:
			self.currentTable += 1
			self.xmlcode.toTables()
		
	def endTable(self, suppress=False):
		self.inTable = False
		if not suppress:
			self.xmlcode.endToTables()
	...
	def buildCaption(self):
		if self.currentcaptionp is not None:
			...
			for l in self.currentcaptionlabels.values():
				caption += "\label{"+l+"}"
			caption += self.xmlcode.getString()
			caption += "}"
			self.xmlcode.reset()
	...
	def buildTableCaption(self):
		if self.currenttablecaptiontbl is not None:
			...
			for l in self.currentcaptionlabels.values():
				tcaption += "\label{"+l+"}"
			tcaption += self.xmlcode.getString()
			#tcaption += "}"
			self.xmlcode.reset()
	...
	def buildTableNote(self):
		if self.currenttablenotetbl is not None:
			tablenote = "\\tnote[]{"
			tablenote += self.xmlcode.getString()
			tablenote += "}%"
			self.xmlcode.reset()
...
class docx2xelatex:
	def __init__(self, docx):
		...
		self.xmlcode = self.config.xmlcode
		
		print()
		print("Converting {:s} to xml".format(self.docxfilename))
	...
	def writeLaTeXFiles(self):
		self.xmlcode.toFile(self.basefilename)
...
class ParagraphProcessor:
	def __init__(self, parentprocessor, p):
		...
		self.xmlcode = self.parentprocessor.xmlcode
		self.process()
	...
	def endHeading(self):
		...
		if self.p in self.config.bookmarks:
			h = self.config.style2heading[self.p.getStyle()]
			self.xmlcode.append("\\label{"+h+":"+self.config.bookmarks[self.p]["name"]+"}")
			self.xmlcode.addNL()
	...
	def startList(self):
		...
		self.xmlcode.addNL()
		if listType == 'bullet':
			self.xmlcode.append('\\begin{itemize}')
		elif listType == 'decimal':
			self.xmlcode.append('\\begin{enumerate}')
		elif listType == 'lowerRoman':
			self.xmlcode.append('\\begin{enumerate}[(i)]')
		elif listType == 'lowerLetter':
			self.xmlcode.append('\\begin{enumerate}[(a)]')
		else:
	...
	def startEnvironmentIfNeeded(self):
		if self.p.isFirstWithEnvironment():
			self.xmlcode.append("\\begin{" + self.p.getEnvironment(self.config.style2environment) + "}")
	
	def endEnvironmentIfNeeded(self):
		if self.p.isLastWithEnvironment():
			self.xmlcode.append("\\end{" + self.p.getEnvironment(self.config.style2environment) + "}")
	...
	def process(self):
		...
		if self.p.isHeading(self.config.style2heading):
			...
			if self.p in self.config.captions.keys():
				self.config.xmlcode.append(self.config.captions[self.p])
				self.xmlcode.addNL()
...
class RunProcessor:
	...
	def startStyleIfNeeded(self, s):
		if self.r.isFirstWithStyleProperty(s):
			self.xmlcode.append('\\' + self.config.styleproperty2command[s] + '{')
	...
	def startStrangeScript(self, language):
		...
		self.xmlcode.append("\\begin{" + self.config.inStrangeScript + "}")
		
	def stopStrangeScript(self):
		self.xmlcode.append("\\end{" + self.config.inStrangeScript + "}")
		...
		if len(self.r) > 0:
			...
			while node is not None:
				if node.tag == W+'t':
					...
					self.xmlcode.append("\-") # Maybe not: this prevents hyphenation elsewhere in the word
...
class LaTeXizer:
	...
	def process(self):
		...
		for c in self.s:
			if self.config.inStrangeScript:
...
class FootnoteProcessor:
	...
	def process(self):		
		...
		self.xmlcode.append('\\footnote{')
		
		if fid in self.config.bookmarks:
			self.xmlcode.append("\\label{footnote:"+self.config.bookmarks[fid]["name"]+"}")
	...
	def process(self):
		...
		if not suppressTable:
			...
			if self.config.inFootnote:
				...
				self.xmlcode.append("\\ctable[%")
				self.xmlcode.addNL()
				self.xmlcode.append("\twidth="+tablewidth+",%")
				self.xmlcode.addNL()
				self.xmlcode.append("\tdoinside=\\tablefont\\tiny,%")
				self.xmlcode.addNL()
				self.xmlcode.append("\tcaption={")
				if self.t in self.config.tablecaptions:
					self.xmlcode.append(self.config.tablecaptions[self.t])
			...
			for i in range(ncols):
				self.xmlcode.append("\t>{\\raggedright\\arraybackslash}X%")
				self.xmlcode.addNL()
			self.xmlcode.append("}%")
			self.xmlcode.addNL()
		...
		for r in rows:
			...
			for c in cells:
				...
				if (colspanning > 1) and not suppressTable:
					self.xmlcode.append('\\multicolumn{'+str(colspanning)+'}{c}{')
				
				# check for rowspans
				rowspanning = c.getRowSpanning()
				if (rowspanning > 1) and not suppressTable:
					self.xmlcode.append("\\multirow{"+str(rowspanning)+"}*{")
				
				# check cell color
				if self.config.useTableColors:
					ccolor = c.getBackgroundColor()
					if ccolor is not None:
						self.xmlcode.addColor(ccolor)
						self.xmlcode.append("\\cellcolor{RGB"+ccolor+"} ")
			...
			if not suppressTable:
				self.xmlcode.append("\\tabularnewline ")
				if rowcounter == 0 and nrows >1:
					self.xmlcode.append("\\midrule")
				self.xmlcode.addNL()
			rowcounter += 1
		
		# End table
		if not suppressTable:
			self.xmlcode.append("\\bottomrule")
			self.xmlcode.addNL()
		
			if self.config.inFootnote:
				self.xmlcode.append("\\end{tabularx}}}")
			else:
				self.xmlcode.append("}}")
...
class MathParagraphProcessor:
	...
	def process(self):
		...
		self.xmlcode.addNL()
		self.xmlcode.addNL()
		self.xmlcode.append("\\begin{equation}")
		self.xmlcode.addNL()
		
		if self.mp in self.config.bookmarks:
			self.xmlcode.append("\\label{equation:"+self.config.bookmarks[self.mp]["name"]+"}")
			self.xmlcode.addNL()
		...
		self.xmlcode.addNL()
		self.xmlcode.append("\\end{equation}")
		self.xmlcode.addNL()
...
class MathRunProcessor:
	...
	def process(self):
		...
		while node is not None:
			if node.tag == M + 't':
				...
				self.xmlstring += mtp.getXeLaTeXString()
	...
	def startStyleIfNeeded(self, s):
		if self.r.isFirstWithStyleProperty(s):
			self.xmlstring += "\\" + self.config.styleproperty2command[s] + "{"
...
class MathTextProcessor:
	...
	def process(self):
		...
		self.xmlstring += l.getXeLaTeXString()
...
class DrawingProcessor:
	...
	def process(self):
		...
		if refID is None:
			...
			self.xmlcode.addNL()
			#self.xmlcode.append("%% Drawing omitted")
			self.xmlcode.addNL()
			#return	
		else:
		...
		if commented:
			self.xmlcode.append("%")
		self.xmlcode.append("\\begin{")
		self.xmlcode.append("figure")
		self.xmlcode.append("}")
		self.xmlcode.addNL()
		if commented:
			self.xmlcode.append("%")
		if latexname is not None:
			self.xmlcode.append("\\includegraphics[width={:.2f}cm]".format(sx)+"{"+latexname+"}")
		else:
			self.xmlcode.append("%% Drawing omitted")
		self.xmlcode.addNL()
		...
		if self.d in self.config.captions:
			self.xmlcode.append("\\caption{")
			self.xmlcode.append(self.config.captions[self.d])
			self.xmlcode.append("}")
			self.xmlcode.addNL()
...
class BookmarkProcessor:
	...
	def selectEquation(self, blink):
		...
		self.xmlcode.startToString()
		MathParagraphProcessor(self, prevEq)
		self.xmlcode.endToString()
		prevEqCode = self.xmlcode.getString()
		print()
		print("--------------")
		print("Equation 1:")
		print(prevEqCode)
		...
		self.xmlcode.endToString()
		nextEqCode = self.xmlcode.getString()
		print("--------------")
		print("Equation 2:")
		print(nextEqCode)
			...
			if refname in self.config.labels:
				...
				for rt in rtext:
					if rt in self.config.floatnames:
						self.xmlcode.append(rt+"~")
						break
				self.xmlcode.append("\\ref{"+self.config.labels[refname]+"}")
			else:
...
class CaptionProcessor:
	...
	def process(self):
		...
		if self.cp.isFirstWithStyle():
			...
			self.config.xmlcode.startToString()
