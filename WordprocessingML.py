...
class body(WordprocessingMLElement):
    ...
    #
    # Child elemens: altChunk, bookmarkEnd, bookmarkStart, commentRangeEnd, commentRangeStart, customXml,
...
class tbl(WordprocessingMLElement):
    ...
    def getColumnContentAlignment(self, columnnumber):
        alignments = []
        row = self.getRow(0)
        while row is not None:
            cell = row.getCell(columnnumber)
            alignments.append(cell.getBorder(side))
            row = row.getNextRow()
...
class tc(WordprocessingMLElement):
    ...
    def getShadingDict(self):
        ...
        if tcPr is not None:
            ...
            if tcShading is not None:
                ...
                #print "Found shading", val, color, fill
                return {'val':val, 'color':color, 'fill':fill}
        return None
    ...
    def getColumnSpanning(self):
        ...
        if cp is not None:
            ...
            if hm is not None:
                raise DocxXMLError("Column spanning with hMerge!")
...
class sym(WordprocessingMLElement):
    ...
    def getSymbolCode(self):
        c = self.get('char')
        # If code point is in the private use area, take out the f000 part !!!! Not anymore! Let XeLaTeXUnicodeLibrary handle PUA
        return int(c, 16) #%int('f000', 16)
...
class bookmarkStart(WordprocessingMLElement):
    ...
    def getBookmarkType(self, headingstyles, floatnames, captionstyles):
        ...
        #print("Heading: "+str(hasHeading)+"; footnote: "+str(hasFootnote)+"; caption: "+str(hasCaption)+"; equation: "+str(hasEquation))
        #input()
    ...
    def isFootnote(self, node):
        if node.tag == W+'footnoteReference':
            ...
            #print("Footnote "+str(self.footnoteID))
            return True
        return False
