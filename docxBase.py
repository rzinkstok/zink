...
class DocX:
	...
	def checkFilename(self, filename):
		...
		if filename is None or not os.path.exists(filename):
			raise DocxFileError("Invalid or empty file name "+filename)
	...
	#	return self.documentxml.getHeadings(styles)
...
class FootnotesXML(DocxXML):
	...
	def getFootnote(self, fid):
		...
		#print "Searching for footnote id", fid, "out of", len(fns)-2, "footnotes"
		for fn in fns:
			if fn.get('id') == fid:
				return fn
		return None
...
class NumberingXML(BaseDocxXML):
	...
	def resolveAbstractNumberingDefinition(self, anumid):
		#print "Resolving abstract numbering definition", anumid
